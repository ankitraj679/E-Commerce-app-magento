...
import { ShippingMethod, CountryPicker } from "@components";
...
class Delivery extends PureComponent {
      ...
      <View style={formGroupStyle}>
        ...
        <CountryPicker
          ...
          <Text
            ...
            {locals.value == "" ? locals.placeholder : countries[locals.value]}
          </Text>
        </CountryPicker>
        {help}
        {error}
  ...
  fetchCustomer = async (props) => {
    ...
    this.setState({ value, cca2: value.country });
  };
  ...
  updateShippingMethod = (value) => {
    this.props.getShippingMethod({ country_id: value.cca2 }, this.props.quote_id, this.props.user.token)
  }

  render() {
    const { shippings, shippingMethod } = this.props;
    const isShippingEmpty = shippingMethod == null || typeof shippingMethod.id === "undefined";
    const {
      theme: {
        colors: { text },
      },
              ...
              <ScrollView contentContainerStyle={styles.shippingMethod}>
                {shippings.map((item, index) => (
                  <ShippingMethod
                    ...
                    selected={shippingMethod && item.id == shippingMethod.id}
                  />
                ))}
              </ScrollView>
            </View>
