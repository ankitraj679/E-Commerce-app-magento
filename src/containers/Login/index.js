...
class LoginScreen extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    // check case after logout
    if (user) {
      if ((!oldUser.user || (params && params.onCart)) && this.isRequesting) {
  ...
  onLoginPressHandle = async () => {
    ...
    MagentoWorker.login(username, password)
      .then((res) => {
  ...
  onFBLoginPressHandle = () => {
    ...
    FacebookAPI.login()
      .then(async (token) => {
        if (token) {
          MagentoWorker.socialLogin(token, "facebook")
            .then((res) => {
              login(res.user, res.token);
            })
            .catch((message) => {
  ...
  onGoogleLoginPressHandle = () => {
    const { login } = this.props;
    Google.logInAsync({
      clientId: Config.GoogleSignIn.iosClientId,
      ...
      if (result.type === "success") {
        ...
        this.isRequesting = true
        MagentoWorker.socialLogin(token, "google")
          .then((res) => {
            login(res.user, res.token);
          })
          .catch((message) => {
      ...
      <KeyboardAwareScrollView
        ...
        <View style={styles.logoWrap}>
          <Image
            ...
            style={[styles.logo, { tintColor: text }]}
            resizeMode="contain"
          />
        </View>
        <View style={styles.subContain}>
          ...
          <ButtonIndex
            ...
            containerStyle={styles.ggButton}
            onPress={this.onGoogleLoginPressHandle}
          />
