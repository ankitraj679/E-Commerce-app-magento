...
class Detail extends PureComponent {
    ...
    return (
      <View style={[styles.tabView, { backgroundColor: background }]}>
        ...
        {this.state.tabIndex === 2 && (
          <VideoTab product={this.props.product} />
        )}
      </View>
    );
  };
  ...
  render() {
    ...
    const in_stock = product.stock_item && product.stock_item.qty > 0
    const isLoading = !product.stock_item
        ...
        <View
          ...
          {!Config.HideCartAndCheckout && (
            <Button
              ...
              disabled={!in_stock}
              buttonStyle={styles.buttonStyle}
              onPress={() => in_stock && this.addToCart(true)}
            />
          )}
        </View>
        {!Config.HideCartAndCheckout && (
          <Button
            text={isLoading ? Languages.Loading : in_stock ? Languages.BUYNOW : Languages.OutOfStock}
            style={[styles.btnBuy, !isLoading && !in_stock && styles.outOfStock]}
            textStyle={styles.btnBuyText}
            disabled={!in_stock || isLoading}
            onPress={() => {
              in_stock && this.addToCart(true);
            }}
          />
        )}
      </View>
    ...
    const renderTitle = () => (
      <View style={{ justifyContent: "center", marginTop: 6, marginBottom: 8 }}>
        ...
        <Text style={[styles.productPrice, { color: text, textAlign: 'center', marginVertical: 10 }]}>{product.id}</Text>
        <View
          style={{
            flexDirection: "row",
