...
import { Languages, Color, Tools, Config, withTheme, GoogleAnalytics } from "@common";
...
class UserProfile extends PureComponent {
  ...
  _onToggleDarkTheme = () => {
    let value = this.props.isDarkTheme ? 1 : 2;
    GoogleAnalytics.strackEvent("Dark Theme", "Change", "Change Dark Theme", value);
  ...
  _handlePress = (item) => {
    ...
    if (routeName && !isActionSheet) {
      GoogleAnalytics.strack(routeName);
      navigation.navigate(routeName, item.params);
    }

    if (isActionSheet) {
      GoogleAnalytics.strackEvent("Currency", "Choose currency", "Choose currency", 1);
    ...
    return (
      <View style={[styles.container, { backgroundColor: background }]}>
        <ScrollView ref="scrollView">
          <UserProfileHeader
            onLogin={() => {
              GoogleAnalytics.strack("LoginScreen");
