...
export const actions = {
  fetchProductsByCategoryId: async (
    ...
    MagentoWorker.productsByCategoryId(
      categoryId,
      per_page,
      page,
      filters
  ...
  fetchProductsByName: async (
    ...
    MagentoWorker.productsByName(name, per_page, page, filter)
      .then((items) => {
        dispatch({
          type:
            page == 1
  ...
  fetchProductRelated: async (dispatch, product) => {
    ...
    if (categories.length > 0) {
      ...
      MagentoWorker.productsByCategoryId(categoryId, 10, 1)
        .then((items) => {
          dispatch({
  ...
  switch (type) {
    ...
    case types.FETCH_PRODUCTS_SUCCESS: {
      return Object.assign({}, state, {
        ...
        stillFetch: items.length == Constants.pagingLimitProducts,
        error: null,
        productFinish: finish,
      });
    }
