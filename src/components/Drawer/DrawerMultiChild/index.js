...
class DrawerMultiChild extends PureComponent {
  ...
  onPressCategory = (category) => {
    if (category.children_data.length > 0) {
      ...
      this.props.setSelectedCategory(params);
      this.props.goToScreen("CategoryDetail", params, false);
    }
  }

  renderItem = (item) => {
    const { colorTextMenu } = this.props;
    const textStyle = { marginLeft: (item.level - 1) * (item.level > 2 ? 20 : 30) }
    return (
      <TouchableOpacity onPress={() => this.onPressCategory(item)}>
        <Text style={[styles.category, textStyle, { color: colorTextMenu }]}>{item.name}</Text>
      </TouchableOpacity>
    )
  }
  ...
  _handlePress = (item, section) => {
    ...
    if (section) {
      ...
      goToScreen(item.routeName, item.params);
    }
  };
